"""Constants for the Emaldo client.

APP_ID, APP_SECRET, and DEFAULT_APP_VERSION are loaded from
``.emaldo_params.json`` (generated by ``python -m emaldo.extract_keys``) when needed.
"""
...
_PARAMS_LOCATIONS = [
    _os.path.join(_os.getcwd(), ".emaldo_params.json"),
    _os.path.join(_os.path.expanduser("~"), ".emaldo_params.json"),
    _os.path.join(_os.path.dirname(_os.path.abspath(__file__)), ".emaldo_params.json"),
]
...
def _load_params():
    if _override_params is not None:
        return _override_params
    for path in _PARAMS_LOCATIONS:
        if _os.path.exists(path):
            with open(path) as _f:
                return _json.load(_f)
    raise RuntimeError(
        "Missing .emaldo_params.json in any of: "
        + ", ".join(_PARAMS_LOCATIONS)
        + "\nRun:  python -m emaldo.extract_keys <path/to/base.apk> --update"
    )

def get_app_id() -> str:
    return _load_params()["app_id"]

def get_app_secret() -> bytes:
    return _load_params()["app_secret"].encode()

def get_default_app_version() -> str:
    return _load_params()["app_version"]

# App identity (from .emaldo_params.json)
# Use get_app_id(), get_app_secret(), get_default_app_version() to access these values.
...
def price_unit_for_timezone(tz_name: str) -> str:
    ...
    Returns ``öre/kWh`` for Sweden, ``øre/kWh`` for Norway/Denmark,
    and ``c/kWh`` (euro cents) for all other timezones.
    """
...
def decode_slot_action(value: int, low: int, high: int) -> str:
    ...
    Returns:
        String like ``charge-low (20%)``, ``discharge-high (72%)``,
        ``idle``, ``none``, or ``charge (N%)``.
    """
    ...
    if value > 128:
        # Discharge threshold (two's complement)
        threshold = 256 - value
        if threshold == low:
